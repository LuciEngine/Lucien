...
impl Texture {
    pub fn new(path: &str, device: &wgpu::Device, queue: &wgpu::Queue) -> Self {
        ...
        match TextureExt::upload_to_gpu(
            &diffuse_texture, &diffuse_rgba, texture_size, queue)
            {
                Err(e) => { eprintln!("{:?}", e); panic!("Failed to upload texture."); }
                _ => { println!("done."); }
            };
...
impl TextureExt {
    ...
    pub(super) fn view(texture: &wgpu::Texture, device: &wgpu::Device) -> (wgpu::TextureView, wgpu::Sampler) {
